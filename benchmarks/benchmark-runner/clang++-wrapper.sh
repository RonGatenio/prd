...
EXTRA_FLAGS="-fpass-plugin=/app/build/bin/PrdPass.so -mclwb -mclflushopt"

# Flags to append during linking
LINK_FLAGS="-fuse-ld=gold -z muldefs -l:/app/build/bin/cprd-x86_64.a -lm -lstdc++ -pthread"
...
if [[ "$ENABLE_PRD" == "true" ]]; then
    ...
    if is_linking "$@"; then
        # Linking stage
        exec "$REAL_CLANGXX" "$@" $EXTRA_FLAGS $LINK_FLAGS
    else
        # Compilation stage
        exec "$REAL_CLANGXX" "$@" $EXTRA_FLAGS
    fi
else
