...
class PersistencyRaceDetector:
    ...
    def _get_first_dependent(self, read_node: nodes.InstructionNode) -> nodes.InstructionNode:
        return next(self.ppdg.get_dependants(read_node))
    
    class VectorClock:
        def __init__(self, hbg: HBG, tid: int):
            self._hbg = hbg
            self._tid = tid
            self._found      : Dict[ThreadId, Dict[Var, nodes.InstructionNode]] = defaultdict(lambda: defaultdict(lambda: None))
            self._persistent : Dict[ThreadId, Dict[Var, nodes.InstructionNode]] = defaultdict(lambda: defaultdict(lambda: None))

        def merge(self, other: 'PersistencyRaceDetector.VectorClock'):
            for tid, var_dict in other._found.items():
                pass
